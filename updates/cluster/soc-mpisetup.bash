#!/bin/bash
# Max Narvaez, Dick Brown, Tanaka Khondowe
...
## Using nmap (“Network Mapper”) an open source tool designed to rapidly scan large networks.
## Switched from fping to nmap since nmap is more reliable
## nmap works by sending ARP (Address Resolution Protocol) requests
## fping sends out ICMP pings which some hosts may not respond to, if configured so
## See https://layeredsecurity.wordpress.com/2018/05/20/nmap-and-fping-deep-dive/ for a quick
## comparison
## This is possibly the reason why fping was failing to pick up some hosts on the cluster.
## nmap is not noticeably faster than fping but it is very consisten and thus more reliable
## The option -sn (No port scan), does a "ping scan" which prints out nodes that are available
## on the network.

IPS=$(nmap -sn 172.27.1.2/24 2>&1 | grep "172.27.1.*" | cut -d " " -f 5)
